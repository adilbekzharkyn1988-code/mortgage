 ...
 * (lib/affordability.ts). Gemini используется отдельно (см.
 * lib/ai/programRecommendation.ts) только чтобы объяснить и ранжировать
 * УЖЕ посчитанный здесь список — не для самого расчёта пригодности.
 */
